...
import { BackupView, CustomersView, DEFAULT_SETTINGS, PrintCenter, ReportsView, SettingsView } from "@/features/tools/tools-views"
...
function NavContent({ view, setView }: { view: AppView; setView: (view: AppView) => void }) { return <nav aria-label="التنقل الرئيسي" className="flex flex-col gap-1">{navigation.map(({ id, label, icon: Icon }) => <button key={id} type="button" onClick={() => setView(id)} data-active={view === id} className="nav-item flex min-h-11 items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-sidebar-foreground/65 transition-colors hover:bg-sidebar-accent hover:text-sidebar-accent-foreground data-[active=true]:bg-sidebar-primary data-[active=true]:text-sidebar-primary-foreground"><Icon />{label}</button>)}</nav> }
...
export function TailorApp() {
  ...
  return <div className="min-h-dvh bg-background">
    ...
    <div className="lg:pr-64"><header className="sticky top-0 z-30 border-b bg-background/95 backdrop-blur-xl"><div className="flex h-16 items-center justify-between gap-3 px-4 sm:px-6 lg:px-8"><div className="flex items-center gap-3"><Button variant="ghost" size="icon" className="lg:hidden" onClick={() => setMobileOpen(true)} aria-label="فتح القائمة"><Menu /></Button><div><p className="font-bold">خياطتي</p><p className="text-xs text-muted-foreground">{navigation.find((item) => item.id === view)?.label}</p></div></div><div className="flex items-center gap-2">{!online && <Badge variant="outline"><CloudOff />دون اتصال</Badge>}{installPrompt && <Button variant="outline" size="sm" onClick={async () => { await installPrompt.prompt(); setInstallPrompt(null) }}><Download /><span className="hidden sm:inline">تثبيت</span></Button>}<Button className="hidden sm:flex" size="sm" onClick={add}><Plus />عملية جديدة</Button></div></div></header>
      ...
      <main className="mx-auto min-h-[calc(100dvh-4rem)] max-w-[1500px] p-4 pb-32 sm:p-6 sm:pb-32 lg:p-8 lg:pb-8">{view === "dashboard" && <DashboardView operations={operations} onAdd={add} onViewAll={() => setView("operations")} onNavigate={setView} />}{view === "operations" && <OperationsView operations={operations} onAdd={add} onEdit={edit} onPrint={(operation) => printOperations([operation], settings, `عملية ${operation.pieceType}`)} />}{view === "customers" && <CustomersView operations={operations} />}{view === "reports" && <ReportsView operations={operations} />}{view === "print" && <PrintCenter operations={operations} settings={settings} />}{view === "backup" && <BackupView settings={settings} />}{view === "settings" && <SettingsView settings={settings} />}</main></div>
    ...
    <nav aria-label="التنقل على الهاتف" className="fixed inset-x-3 bottom-[calc(.75rem+env(safe-area-inset-bottom))] z-40 grid grid-cols-5 items-center rounded-2xl border bg-background/95 p-1.5 shadow-lg backdrop-blur-xl lg:hidden">{mobileNavigation.map(({ id, label, icon: Icon }) => <button key={id} type="button" onClick={() => setView(id)} data-active={view === id} className="flex min-h-12 flex-col items-center justify-center gap-1 rounded-xl px-1 text-[10px] text-muted-foreground data-[active=true]:bg-primary data-[active=true]:text-primary-foreground"><Icon />{label}</button>)}<button type="button" onClick={() => setMobileOpen(true)} className="flex min-h-12 flex-col items-center justify-center gap-1 rounded-xl px-1 text-[10px] text-muted-foreground"><Menu />المزيد</button></nav>
    <Sheet open={mobileOpen} onOpenChange={setMobileOpen}><SheetContent side="right" dir="rtl"><SheetHeader><SheetTitle className="flex items-center gap-2"><Scissors />{settings.shopName}</SheetTitle><SheetDescription>كل أدوات العمل والنسخ والطباعة.</SheetDescription></SheetHeader><div className="p-4"><NavContent view={view} setView={navigate} /></div></SheetContent></Sheet><OperationDialog open={dialogOpen} onOpenChange={setDialogOpen} operation={editing} />
  </div>
}
