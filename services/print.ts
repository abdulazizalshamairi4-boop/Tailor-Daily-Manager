...
export async function downloadOperationsPdf(operations: TailorOperation[], settings: AppSettings, title: string) {
  const container = document.createElement("div")
  container.style.cssText = "position:fixed;left:-10000px;top:0;width:794px;background:white"
  const html = buildPrintDocument(operations, settings, title).match(/<body>([\s\S]*)<script>/)?.[1] ?? ""
  container.innerHTML = html; document.body.appendChild(container)
  const [{ default: html2canvas }, { jsPDF }] = await Promise.all([import("html2canvas"), import("jspdf")])
  const canvas = await html2canvas(container, { scale: 2, backgroundColor: "#ffffff" })
  const pdf = new jsPDF("p", "mm", "a4"); const width = 190; const height = canvas.height * width / canvas.width; const pageHeight = 277
  const image = canvas.toDataURL("image/jpeg", .92)
  let offset = 0; let page = 0
  while (offset < height) { if (page) pdf.addPage(); pdf.addImage(image, "JPEG", 10, 10 - offset, width, height); offset += pageHeight; page++ }
  pdf.save(`${title}-${new Date().toISOString().slice(0, 10)}.pdf`); container.remove()
}
